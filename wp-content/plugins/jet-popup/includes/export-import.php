	...
	class Jet_Export_Import {
		...
		public function prepare_popup_export( $popup_id ) {
			...
			if ( ! empty( $popup_conditions ) ) {
				$popup_data['_conditions'] = $popup_conditions;
            }

			$popup_relation_type = get_post_meta( $popup_id, '_relation_type', true );

			if ( ! empty( $popup_relation_type ) ) {
				$popup_data['_relation_type'] = $popup_relation_type;
			}
		...
		public function import_popup_preset() {
			...
			$post_data = array(
				...
				'meta_input' => array(
                    ...
                    '_conditions'              => isset( $content['_conditions'] ) ? $content['_conditions'] : [],
                    '_relation_type'           => isset( $content['_relation_type'] ) ? $content['_relation_type'] : [],
					'_content_type'            => 'elementor',
					'_settings'                => jet_popup()->settings->merge_with_defaults_settings( $page_settings ),
				),
			);
            ...
            $popup_conditions = isset( $popup_settings['jet_popup_conditions'] ) ? $popup_settings['jet_popup_conditions'] : [];

			jet_popup()->conditions_manager->update_site_popup_conditions( $popup_id, $popup_conditions, 'or' );
